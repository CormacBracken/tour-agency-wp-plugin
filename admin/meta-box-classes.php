...
class AT_Meta_Box {
  ...
  public function check_field_date() {
    ...
    if ( $this->has_field( 'date' ) && $this->is_edit_page() ) {
	  ...
	  $plugin_path = $this->SelfPath;
      wp_enqueue_style( 'at-jquery-ui-css', plugin_dir_url( __FILE__ ) .'js/jquery-ui/jquery-ui.css' );
  ...
  public function check_field_time() {
    ...
    if ( $this->has_field( 'time' ) && $this->is_edit_page() ) {
      ...
      wp_enqueue_style( 'at-jquery-ui-css', plugin_dir_url( __FILE__ ) .'js/jquery-ui/jquery-ui.css' );
      wp_enqueue_script( 'jquery-ui');
      wp_enqueue_script( 'at-timepicker', plugin_dir_url( __FILE__ ) .'js/jquery-ui/jquery-ui-timepicker-addon.js', array( 'jquery-ui-slider','jquery-ui-datepicker' ),false,true );
    }
  }
  ...
  public function check_field_code() {
    ...
    if ( $this->has_field( 'code' ) && $this->is_edit_page() ) {
      ...
      wp_enqueue_style( 'at-code-css', plugin_dir_url( __FILE__ ) . 'js/codemirror/codemirror.css',array(),null);
      wp_enqueue_style( 'at-code-css-dark', plugin_dir_url( __FILE__ ) . 'js/codemirror/solarizedDark.css',array(),null);
      wp_enqueue_style( 'at-code-css-light', plugin_dir_url( __FILE__ ) . 'js/codemirror/solarizedLight.css',array(),null);
      wp_enqueue_script('at-code-js', plugin_dir_url( __FILE__ ) . 'js/codemirror/codemirror.js',array('jquery'),false,true);
      wp_enqueue_script('at-code-js-xml', plugin_dir_url( __FILE__ ) . 'js/codemirror/xml.js',array('jquery'),false,true);
      wp_enqueue_script('at-code-js-javascript', plugin_dir_url( __FILE__ ) . 'js/codemirror/javascript.js',array('jquery'),false,true);
      wp_enqueue_script('at-code-js-css', plugin_dir_url( __FILE__ ) . 'js/codemirror/css.js',array('jquery'),false,true);
      wp_enqueue_script('at-code-js-clike', plugin_dir_url( __FILE__ ) . 'js/codemirror/clike.js',array('jquery'),false,true);
      wp_enqueue_script('at-code-js-php', plugin_dir_url( __FILE__ ) . 'js/codemirror/php.js',array('jquery'),false,true);
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $meta = get_post_meta($post->ID,$field['id'],true);
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        echo '</table>';
        if ($field['sortable'])
          echo '<span class="re-control"><img src="'.plugin_dir_url( __FILE__ ).'images/move.png" alt="sort" title="sort" class="at_re_sort_handle" /></span>';

        echo'
        <span class="re-control at-re-toggle"><img src="'. plugin_dir_url( __FILE__ ) .'images/edit.png" alt="Edit" title="Edit"/></span> 
        <span class="re-control"><img src="'.plugin_dir_url( __FILE__ ).'images/remove.png" alt="'.__('Remove','mmb').'" title="'.__('Remove','mmb').'" id="remove-'.$field['id'].'"></span>
    ...
    if ($this->_Local_images){
      echo plugin_dir_url( __FILE__ ).'images/add.png';
    }else{
      echo 'http://i.imgur.com/w5Tuc.png'; //TODO: get rid of non-local images option
    }
    ...
    echo '</table><img src="'. plugin_dir_url( __FILE__ ) .'images/remove.png" alt="'.__('Remove','mmb').'" title="'.__('Remove','mmb').'" id="remove-'.$field['id'].'"></div>';
