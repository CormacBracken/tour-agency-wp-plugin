(function( $ ) {
	...
	$( '.cmb-type-cb-multimap' ).each( function() {
		initializeMap( $( this ) );
	});

	function initializeMap( mapInstance ) {
		var searchInput = mapInstance.find( '.cb-multimap-search' );
		var mapCanvas = mapInstance.find( '.cb-multimap' );
		var latitude = mapInstance.find( '.cb-multimap-latitude' );
		var longitude = mapInstance.find( '.cb-multimap-longitude' );
	...
	$( '.cmb-repeatable-group' ).on( 'cmb2_add_row', function( event, newRow ) {
		...
		groupWrap.find( '.cmb-type-cb-multimap' ).each( function() {
			initializeMap( $( this ) );
		});
	});
